...
void pushStatic(StaticStack *s, int x) {
    if (isStaticFull(s)) { printf("Overflow (static)\n"); return; }
    s->a[++s->top] = x;
}

int popStatic(StaticStack *s) {
    if (isStaticEmpty(s)) { printf("Underflow (static)\n"); return -1; }
    return s->a[s->top--];
}

int peekStatic(StaticStack *s) {
    if (isStaticEmpty(s)) { printf("Empty (static)\n"); return -1; }
    return s->a[s->top];
}
...
int popDynamic(DynamicStack *s) {
    if (isDynamicEmpty(s)) { printf("Underflow (dynamic)\n"); return -1; }
    Node *temp = s->top;
    int x = temp->data;
    s->top = temp->next;
    free(temp);
...
int peekDynamic(DynamicStack *s) {
    if (isDynamicEmpty(s)) { printf("Empty (dynamic)\n"); return -1; }
    return s->top->data;
}
...
void displayDynamic(DynamicStack *s) {
    ...
    for (Node *p = s->top; p != NULL; p = p->next) printf("%d ", p->data);
    printf("\n");
}

void freeDynamic(DynamicStack *s) {
    while (!isDynamicEmpty(s)) popDynamic(s);
}
...
void staticMenu(StaticStack *s) {
    ...
    do {
        ...
        scanf("%d", &ch);
        switch (ch) {
            case 1: printf("Value: "); scanf("%d", &x); pushStatic(s, x); break;
            case 2: x = popStatic(s); if (x != -1) printf("Popped: %d\n", x); break;
            case 3: x = peekStatic(s); if (x != -1) printf("Top: %d\n", x); break;
            case 4: displayStatic(s); break;
        }
    } while (ch != 5);
}
...
void dynamicMenu(DynamicStack *s) {
    ...
    do {
        ...
        scanf("%d", &ch);
        switch (ch) {
            case 1: printf("Value: "); scanf("%d", &x); pushDynamic(s, x); break;
            case 2: x = popDynamic(s); if (x != -1) printf("Popped: %d\n", x); break;
            case 3: x = peekDynamic(s); if (x != -1) printf("Top: %d\n", x); break;
            case 4: displayDynamic(s); break;
        }
    } while (ch != 5);
}
...
int main(void) {
    StaticStack  sArr;   initStatic(&sArr);
    DynamicStack sList;  initDynamic(&sList);
    ...
    do {
        ...
        scanf("%d", &ch);
        switch (ch) {
            case 1: staticMenu(&sArr); break;
            case 2: dynamicMenu(&sList); break;
        }
    } while (ch != 3);
